...
class YOLOXONNX(object):
    def __init__(
        ...
        self.input_shape=tuple(map(int, yolox_config["input_shape"].split(',')))
        self.class_score_th=yolox_config["class_score_thr"]
        self.with_p6=yolox_config["with_p6"]

        if yolox_config["device"] == 'cpu':
            providers=['CPUExecutionProvider']
        else:
            providers=['CUDAExecutionProvider', 'CPUExecutionProvider']

        self.onnx_session = onnxruntime.InferenceSession(
            yolox_config["model_path"],
            providers=providers,
        )
    
    def inference(self, origin_img):
        img, ratio = self._preproc(origin_img, self.input_shape)
        
        ort_inputs = {self.onnx_session.get_inputs()[0].name: img[None, :, :, :]}
        output = self.onnx_session.run(None, ort_inputs)
        predictions = self._postprocess(output[0], self.input_shape)[0]
        
        result_img = self._visual(origin_img, predictions, ratio)
    ...
    def _postprocess(self, outputs, img_size):
        grids = []
        expanded_strides = []
    ...
    def _visual(self, origin_img, predictions, ratio):
        boxes = predictions[:, :4]
        ...
        if dets is not None:
            ...
            origin_img = self._vis(origin_img, final_boxes, final_scores, final_cls_inds,
                            conf=self.class_score_th, class_names=COCO_CLASSES)
        
        return origin_img
    
    def _multiclass_nms(self, boxes, scores, nms_thr, score_thr, class_agnostic=True):
    ...
    def _vis(self, img, boxes, scores, cls_ids, conf=0.5, class_names=None):
